...
export function useDashboard(apiUrl = "http://192.168.71.200:8080/terrestre2/api_incidencias.php") {
  const cargando   = ref(false)
  const stats      = ref(null)

  const TIPOS = ["asalto","atropellamiento","choque","incendio","inundacion","robo","vehiculo_descompuesto","volcadura"]
  ...
  function _cuadrante(lat, lng) {
    const latN = Number(lat)
    const lngN = Number(lng)
    const ns   = latN >= 19.155 ? "Norte" : "Sur"
    const ew   = lngN >= -96.145 ? "Oriente" : "Poniente"
    return `${ns}-${ew}`
  }
  ...
  async function cargarStats(minutos = 1440) {
    ...
    try {
      ...
      for (const inc of data) {
        ...
        const zona = _cuadrante(inc.latitud, inc.longitud)
        const sev  = Number(inc.severidad) || 1

        porTipo[tipo] = (porTipo[tipo] ?? 0) + 1
        if (!porZona[zona]) porZona[zona] = { total: 0, sevSum: 0, tipos: {} }
        porZona[zona].total++
        porZona[zona].sevSum += sev
        porZona[zona].tipos[tipo] = (porZona[zona].tipos[tipo] ?? 0) + 1
      ...
      const zonasRanking = Object.entries(porZona)
        .map(([nombre, d]) => {
          const tipoDom = Object.entries(d.tipos).sort((a, b) => b[1] - a[1])[0]?.[0] ?? "-"
          const sevProm = d.total ? d.sevSum / d.total : 0
          return { nombre, total: d.total, tipoDom, sevProm, riesgo: _nivelRiesgo(sevProm) }
        })
      ...
      const tipoColores = tipoLabels.map(t => COLORES_TIPO[t]?.color ?? "#888780")

      const tipoMasFrecuente = tipoLabels[tipoCounts.indexOf(Math.max(...tipoCounts))] ?? "-"
      const sevPromGlobal    = data.length ? (sevTotal / data.length).toFixed(1) : "0"
